...
export function RolePermissionsModal({
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const [groupsRes, rolePermsRes] = await Promise.all([
        fetch("/api/permissions"),
        fetch(`/api/roles/${roleId}/permissions`),
      ]);

      if (!groupsRes.ok || !rolePermsRes.ok) {
        throw new Error("Falha ao carregar permissões da função");
      }
      ...
      // Preencher o mapa selectedMap com base em permissionCodes da role
      const initialMap: Record<string, boolean> = {};
      const activeCodes = new Set<string>(rolePermsData.permissionCodes || []);
  ...
  const handleSave = async () => {
    ...
    try {
      const permissionsPayload = Object.entries(selectedMap)
        .filter(([_, granted]) => granted) // Apenas as marcadas para roles
        ...
        <DialogHeader>
          <div className="flex items-center justify-between">
            <DialogTitle className="flex items-center gap-2 text-xl">
              ...
              Matriz de Permissões da Função
            </DialogTitle>
          ...
          <DialogDescription className="text-gray-700 dark:text-gray-300">
            Configure as permissões padrão para todos os usuários que pertencem à função <strong>{roleName}</strong>.
          </DialogDescription>
        </DialogHeader>
